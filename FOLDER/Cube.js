class Cube{
        ...
        ])
    {
        ...
        this.#Points.sort(this.sortForGeneralNumeration);
        this.calculateMiddlePoints()
        this.#MiddlePoints.sort(this.sortForGeneralNumeration)
        
        this.calculateMinMax()
        this.setLocalIDs()

        this.#CnnectionIndices = this.#CnnectionIndices.map(row => row.map(element => element + 8 * (this.#Id - 1)))
    ...
    calculateMiddlePoints(){
        for (let j = 0; j < this.#CnnectionIndices.length; j++){
            for (let i = 0; i < this.#CnnectionIndices[0].length; i += 2) {
                let idx1 = this.#CnnectionIndices[j][i]
                let idx2 = this.#CnnectionIndices[j][i + 1]
    ...
    getCubes_Points_SortedBtGeneral(){
        return this.#Points
    }
    getCubes_MiddlePoints_SortedBtGeneral(){
        return this.#MiddlePoints
    }

    getCubes_AllPoints_SortedBtGeneral(){
        return [
            ...this.getCubes_Points_SortedBtGeneral(),
            ...this.getCubes_MiddlePoints_SortedBtGeneral()
        ].sort(this.sortForGeneralNumeration)
    }

    getCubes_AllPoints_SortedinUsualWay(){
        return [
            ...this.getCubes_Points_SortedBtGeneral(),
            ...this.getCubes_MiddlePoints_SortedBtGeneral()
        ].sort(this.sortForLocalNumeration)
    }
    ...
    sortForLocalNumeration(point1, point2) {
    ...
    getPointsToDraw(){
        let ArrayOfPointsToDrawAs_XYZ_cords = []

        for (let point of this.#Points) {
            ArrayOfPointsToDrawAs_XYZ_cords.push(point.X)
            ArrayOfPointsToDrawAs_XYZ_cords.push(point.Y)
            ArrayOfPointsToDrawAs_XYZ_cords.push(point.Z)
        }

        return ArrayOfPointsToDrawAs_XYZ_cords
    }
    getMiddlePointsToDraw(){
        let ArrayOfMiddlePointsToDrawAs_XYZ_cords = []

        //console.log()
        for (let point of this.#MiddlePoints) {
            ArrayOfMiddlePointsToDrawAs_XYZ_cords.push(point.X)
            ArrayOfMiddlePointsToDrawAs_XYZ_cords.push(point.Y)
            ArrayOfMiddlePointsToDrawAs_XYZ_cords.push(point.Z)
        }
        return ArrayOfMiddlePointsToDrawAs_XYZ_cords
    }
    getIndicesToDraw(){
        let IndicesToDraw = []
        for (let i = 0; i < this.#CnnectionIndices.length; i++)
            IndicesToDraw.push(...this.#CnnectionIndices[i])
    ...
    ChangeCnnectionIndices(i = this.#Id - 1){
        this.#CnnectionIndices = this.#CnnectionIndices.map(row => row.map(element => element + 8 * i));
    }

    get CnnectionIndices(){
        return this.#CnnectionIndices
    }
    ...
    print(){
        ...
        console.log(this.getCubes_AllPoints_SortedBtGeneral())
    }
}